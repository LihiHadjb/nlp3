...
class NerBiLstmModel(torch.nn.Module):
    ...
    def __init__(self, helper, config, pretrained_embeddings):
        ...
        self.bilstm = torch.nn.LSTM(self.config.embed_size*self.config.n_features, int(0.5 * self.config.hidden_size), bidirectional=True)
    ...
    def forward(self, sentences):
        ...
        res = self.embeddings(sentences.type(torch.LongTensor).to(self.config.device)).view(
            (batch_size, seq_length, sentences.shape[2] * self.config.embed_size))
        res = self._dropout(res)
        output, (_, _) = self.bilstm(res)
        res = self._dropout(output)
        res = self.linear(res)
...
class Trainer(TrainerBase):
    ...
    def _batch_loss(self, tag_probs, labels, masks):
        ...
        masked_labels = torch.masked_fill(labels, masks == False, 0).type(torch.LongTensor).to(self._model.config.device)
...
class DataPreprocessor(BaseDataPreprocessor):
    def pad_sequences(self, examples):
        ...
        for sentence, labels in examples:
            ...
            new_sent[:len(sentence)] = sentence
            new_labels[:len(sentence)] = labels
            mask[:len(sentence)] = [True] * len(sentence)
